// Local stand-in for the hosted Sitelemetry MCP endpoint and /api/plans.
// Adapted from the public sitelemetry-audit-action test suite (MIT).
import http from 'node:http';
import { readFileSync } from 'node:fs';

export const TEST_API_KEY = 'sl_test_key_123456';
export const TEST_SESSION_ID = 'sess_mock_0001';
const TOOLS = [
	'audit_security',
	'audit_seo',
	'audit_ai_visibility',
...
export async function startMockServer({ apiKey = TEST_API_KEY, runningPolls = 2, tools = TOOLS } = {}) {
	...
	function handleCall(message, reply, rpcError, res) {
		...
		if (!tools.includes(name)) {
			let unlistedHost;
			try {
				unlistedHost = new URL(args.target).hostname;
			} catch {
				unlistedHost = String(args.target);
			}
			if (unlistedHost === 'plan-result.example') {
				return reply(
					actionRequired(
						'entitlement_required',
						'Audit not started. No audit quota was used. The requested audit requires Starter or higher access and is not included in the connected Free account.',
					),
				);
			}
		...
		if (Object.hasOwn(args, 'jobId')) {
			...
			if (!job || job.tool !== name) {
				return reply(
					actionRequired(
						'audit_job_argument_mismatch',
						'The audit job arguments do not match. Call the same tool with the returned pollArguments unchanged.',
					),
				);
			}
			job.polls += 1;
		...
		let host;
		try {
			host = new URL(args.target).hostname;
		} catch {
			host = String(args.target);
		}
		...
		switch (host) {
			case 'ok.example': {
				...
				const job = {
					...
					polls: 0,
					final: completed,
					pollArguments: { target: 'https://ok.example/', jobId: id },
				};
				jobs.set(id, job);
			...
			case 'free.example':
				return reply(fixture('security-partial-free.json'));
			case 'full.example':
				return reply(fixture('full-partial.json'));
			case 'plan.example':
				return json(res, 402, {
			...
			case 'quota.example':
				return reply(
					actionRequired(
						'usage_limit_reached',
						'Audit not started. No audit quota was used. The current audit allowance is exhausted. Retry after the allowance resets.',
					),
				);
			case 'quota-rpc.example':
				return rpcError(-32603, 'Monthly security scan limit reached for this account (monthly allowance: 10).');
			case 'verify.example':
				return reply(
					actionRequired(
						'target_verification_required',
						'Audit not started. No audit quota was used. Verify ownership of this target in Sitelemetry or Google Search Console before retrying. No plan change is required.',
					),
				);
			case 'consent.example':
				return reply(
					actionRequired(
						...
						'Audit not started. No audit quota was used. Review and accept the current audit authorization terms in your account before retrying.',
					),
				);
			case 'busy.example':
				if (busyCalls++ === 0) {
					return json(
						res,
						429,
						{ error: 'Another audit is already running for this account. Please retry shortly.' },
						{ 'retry-after': '0' },
					);
				}
				return reply(completed);
			case 'error.example':
				return reply({ content: [{ type: 'text', text: 'Error: audit failed' }], isError: true });
			default:
				return reply({ content: [{ type: 'text', text: `Error: unknown fixture host ${host}` }], isError: true });
		}
	}
	...
	function handleMcp(req, res, body) {
		...
		if (bearer !== apiKey) {
			return json(
				res,
				401,
				{ error: 'Unauthorized. Connect with Sitelemetry OAuth or provide a Sitelemetry MCP API key as a Bearer token.' },
				...
				return json(
					...
					200,
					{
						...
						result: {
							...
							serverInfo: { name: 'mock-sitelemetry', version: '0.0.0' },
			...
			case 'tools/list':
				return reply({
					tools: tools.map((name) => ({
						name,
						inputSchema: { type: 'object', properties: { target: { type: 'string' }, jobId: { type: 'string' } } },
					})),
				});
			case 'tools/call':
	...
	return {
		...
		plansCalls: () => calls.filter((call) => call.path === '/api/plans'),
